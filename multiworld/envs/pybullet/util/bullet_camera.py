...
from multiworld.math import Orientation, Pose
...
def create_camera(pybullet_client,
                  height,
                  width,
                  intrinsics,
                  translation ,
    ...
    """
    if  is_simulation:
        camera = BulletCamera(pybullet_client,
                             height=height,
                             width=width,
                             near=near,
                             far=far,
                             ...
                             upside_down=True,
                              ...
                              isImgDepth=isImgDepth,
                              isImgMask=isImgMask,
                              shadow_enable=shadow_enable,
                              opengl_Render_enable=opengl_Render_enable,
                          )
    else:
    ...
    if camera_pose is not None:
        ...
        camera_in_world = Pose([translation, rotation])
        world_origin_in_camera = camera_in_world.inverse()

        translation = world_origin_in_camera.position
        rotation = world_origin_in_camera.euler
...
class BulletCamera(Camera):
    ...
    def __init__(self,
        ...
        """
        self._pybullet_server =  pybullet_server,
        ...
        self._isDepth = isImgDepth
        self.renderer = pybullet.ER_BULLET_HARDWARE_OPENGL if opengl_Render_enable else pybullet.ER_TINY_RENDERER
        self._shadow_flag = 1 if shadow_enable else 0
        self._mask_flags = pybullet.ER_SEGMENTATION_MASK_OBJECT_AND_LINKINDEX if self._isImgMask else pybullet.ER_NO_SEGMENTATION_MASK
    ...
    def _frames(self):
        ...
        _, _, rgba, depth, segmask = pybullet.getCameraImage(
            ...
            shadow=self._shadow_flag, flags=self._mask_flags,
        )
        ...
        if self._upside_down:
            rgb = rgb[::-1, ::-1, :]
            depth = depth[::-1, ::-1] if self._isDepth else None
            segmask = segmask[::-1, ::-1] if self._isImgMask else None
