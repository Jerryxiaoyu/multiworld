...
def draw_image_keypoint(
        ...
        keypoint_pixelxy_depth_true = None,
    ...
    for i in range(n_keypoint):
        ...
        cv2.circle(img_clone,
                   ...
                   radius=8, color=(255, 255, 0))  # blue

        if keypoint_pixelxy_depth_true is not None:
            cv2.circle(img_clone,
                       center=(int(keypoint_pixelxy_depth_true[0, i]), int(keypoint_pixelxy_depth_true[1, i])),
                       radius=8, color=(0, 255, 255)) # yellow
    # OK
    return img_clone
...
def draw_image_bbx(img_rgb, top_left, bottom_right):
    img_clone = img_rgb.copy()
    img_bbx = cv2.rectangle(img_clone, (top_left.x, top_left.y), (bottom_right.x, bottom_right.y), (0, 0, 255), 2)

    # plt.imshow(img_rgb)
    # plt.show()
    return img_bbx

def depthImg2point_cloud( depth):
    """
    points in camera frame
    :param depth: [H, W] numpy from opencv
    :return:  [n, 3]
    """
    camera_name = 'D435i'
    sn = '838212071357'
    ...
    valid = (depth > 0)
    z = np.where(valid, depth / camera_factor, 0)
    x = np.where(valid, z * (c - principal_x) / focal_x, 0)
    y = np.where(valid, z * (r - principal_y) / focal_y, 0)

    points =  np.dstack((x, y, z))
    points = points.reshape(rows*cols,3)
    return points

def depthImg2pcl(depth_img, file_name='cloud'):
    import pcl
    pointcloud_pt = depthImg2point_cloud(depth_img)
    pointcloud = np.array(pointcloud_pt, dtype = np.float32)
    cloud = pcl.PointCloud()
    cloud.from_array(pointcloud)
    pcl.save(cloud, file_name+".pcd", format = 'pcd')
    return cloud
